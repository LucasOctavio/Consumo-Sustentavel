...
def authenticate(nome, senha, session):
    ...
    if not busca.user_verified:
        raise HTTPException(
            status_code=403,
            detail="Conta não verificada. Por favor, acesse seu e-mail para validar a conta."
        )
...
def atualizar_usuario(dados, user_id, session):
    ...
    # Se o usuário quiser mudar o nome, verifica se o novo nome já pertence a outra pessoa
    if dados.user_name:
        existe = session.query(Usuario).filter(
            Usuario.user_name == dados.user_name,
    ...
    # Itera de forma dinâmica pelas chaves e valores passados na requisição (JSON)
...
def renovar_token(busca):
    ...
    # Recebe a identidade (busca) e cria um novo access token a partir do ID do usuário
...
def create_token(data, duracao_token=timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTE)):
    ...
    # Calcula qual será o exato momento em que o token deve expirar
    data_expiracao = datetime.now(timezone.utc) + duracao_token

    # Se o 'data' for um modelo Pydantic, convertemos para dicionário
    if hasattr(data, "dict") and callable(data.dict):
        dic_info = data.dict(exclude_unset=True)
    # Se for um dicionário puro, criamos uma cópia para não alterar a referência original
    elif isinstance(data, dict):
        dic_info = data.copy()
    # Se for apenas um valor (como um ID numérico ou string), encapsulamos em um dicionário
    else:
